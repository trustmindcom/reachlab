...
describe("AI queries", () => {
  ...
  describe("ai_runs", () => {
    ...
    it("getRunningRun returns a running run", () => {
      const id = createRun(db, PERSONA_ID, "manual", 5);
      const running = getRunningRun(db, PERSONA_ID);
      expect(running).not.toBeNull();
      expect(running!.id).toBe(id);
    });

    it("getRunningRun returns null when no running run", () => {
      expect(getRunningRun(db, PERSONA_ID)).toBeNull();
    });
    ...
    it("completeRun sets status and tokens", () => {
      ...
      // getLatestCompletedRun requires an ai_overview record (JOIN)
      upsertOverview(db, {
        run_id: id,
        summary_text: "test",
      ...
      const completed = getLatestCompletedRun(db, PERSONA_ID);
    ...
    it("getLatestCompletedRun returns null when none exist", () => {
      expect(getLatestCompletedRun(db, PERSONA_ID)).toBeNull();
    });

    it("getLatestCompletedRun returns most recent completed", () => {
      const id1 = createRun(db, PERSONA_ID, "manual", 5);
      completeRun(db, id1, {
        input_tokens: 100,
        output_tokens: 50,
        cost_cents: 0.1,
      ...
      upsertOverview(db, {
        run_id: id1, summary_text: "r1", top_performer_post_id: null,
        top_performer_reason: null, quick_insights: "[]", prompt_suggestions_json: null,
      });
      const id2 = createRun(db, PERSONA_ID, "manual", 8);
      completeRun(db, id2, {
        input_tokens: 200,
        output_tokens: 100,
        cost_cents: 0.2,
      ...
      upsertOverview(db, {
        run_id: id2, summary_text: "r2", top_performer_post_id: null,
        top_performer_reason: null, quick_insights: "[]", prompt_suggestions_json: null,
      });
      const latest = getLatestCompletedRun(db, PERSONA_ID);
  ...
  describe("ai_tags", () => {
    ...
    it("getUntaggedPostIds returns posts without ai_tags", () => {
      ...
      const untagged = getUntaggedPostIds(db, PERSONA_ID);
    ...
    it("getUntaggedPostIds returns empty when all tagged", () => {
      ...
      expect(getUntaggedPostIds(db, PERSONA_ID)).toEqual([]);
    });
  });
  ...
  describe("ai_image_tags", () => {
    it("upsertImageTag inserts and retrieves image tags", () => {
      db.prepare("INSERT INTO posts (id, persona_id, content_type) VALUES ('img-test-1', 1, 'image')").run();
    ...
    it("getUnclassifiedImagePosts returns posts with images but no tags", () => {
      db.prepare(
        "INSERT INTO posts (id, persona_id, content_type, image_local_paths) VALUES ('unclass-1', 1, 'image', '[\"data/images/unclass-1/0.jpg\"]')"
      ).run();
      db.prepare(
        "INSERT INTO posts (id, persona_id, content_type, image_local_paths) VALUES ('unclass-2', 1, 'image', '[\"data/images/unclass-2/0.jpg\"]')"
      ).run();
      // Tag one of them
      upsertImageTag(db, {
        post_id: "unclass-2",
      ...
      const unclassified = getUnclassifiedImagePosts(db, PERSONA_ID);
  ...
  describe("insights", () => {
    it("insertInsight and getActiveInsights", () => {
      const runId = createRun(db, PERSONA_ID, "manual", 5);
      ...
      expect(id).toBeGreaterThan(0);
      const active = getActiveInsights(db, PERSONA_ID);
  ...
  describe("recommendations", () => {
    it("insertRecommendation and getRecommendations", () => {
      ...
      upsertOverview(db, {
        run_id: runId, summary_text: "test", top_performer_post_id: null,
        top_performer_reason: null, quick_insights: "[]", prompt_suggestions_json: null,
      });
      ...
      expect(id).toBeGreaterThan(0);
      const recs = getRecommendations(db, PERSONA_ID);
    ...
    it("getRecommendations with explicit runId", () => {
      ...
      const recs = getRecommendations(db, PERSONA_ID, runId);
      expect(recs).toHaveLength(1);
    });

    it("getRecommendations returns empty when no run exists", () => {
      expect(getRecommendations(db, PERSONA_ID)).toEqual([]);
    });
  ...
  describe("ai_overview", () => {
    it("upsertOverview and getLatestOverview", () => {
      ...
      const overview = getLatestOverview(db, PERSONA_ID);
    ...
    it("getLatestOverview returns null when empty", () => {
      expect(getLatestOverview(db, PERSONA_ID)).toBeNull();
    });
  ...
  describe("helpers", () => {
    it("getPostCountWithMetrics counts posts that have metrics", () => {
      ...
      expect(getPostCountWithMetrics(db, PERSONA_ID)).toBe(2);
    });

    it("getPostCountWithMetrics returns 0 when no metrics", () => {
      seedPost("p1");
      expect(getPostCountWithMetrics(db, PERSONA_ID)).toBe(0);
    });

    it("getPostCountSinceRun counts posts published after run completion", () => {
      // Create and complete a run
      const runId = createRun(db, PERSONA_ID, "manual", 2);
      completeRun(db, runId, {
        input_tokens: 10,
        output_tokens: 5,
        cost_cents: 0.01,
      ...
      const count = getPostCountSinceRun(db, PERSONA_ID, runId);
      expect(count).toBe(1);
    });

    it("getChangelog returns categorized insights", () => {
      const run1 = createRun(db, PERSONA_ID, "manual", 5);
      completeRun(db, run1, {
        input_tokens: 10,
        output_tokens: 5,
        cost_cents: 0.01,
      ...
      upsertOverview(db, {
        run_id: run1, summary_text: "r1", top_performer_post_id: null,
        top_performer_reason: null, quick_insights: "[]", prompt_suggestions_json: null,
      });
      const run2 = createRun(db, PERSONA_ID, "manual", 5);
      completeRun(db, run2, {
        input_tokens: 10,
        output_tokens: 5,
        cost_cents: 0.01,
      ...
      upsertOverview(db, {
        run_id: run2, summary_text: "r2", top_performer_post_id: null,
        top_performer_reason: null, quick_insights: "[]", prompt_suggestions_json: null,
      });
      ...
      const changelog = getChangelog(db, PERSONA_ID);
  ...
  describe("writing_prompt_history", () => {
    it("saves and retrieves history entries in reverse chronological order", () => {
      saveWritingPromptHistory(db, PERSONA_ID, {
      ...
      const history = getWritingPromptHistory(db, PERSONA_ID);
  ...
  describe("getLatestPromptSuggestions", () => {
    it("returns null when no completed run exists", () => {
      expect(getLatestPromptSuggestions(db, PERSONA_ID)).toBeNull();
    });
    ...
    it("returns parsed prompt suggestions from latest completed run", () => {
      ...
      const result = getLatestPromptSuggestions(db, PERSONA_ID);
